...
export function getIdealWeight({
  ...
  if (gender === "Male")
    return Math.floor((56.2 + 1.41 * (height * 100 * 0.4)) * 0.45);
  return Math.floor((53.1 + 1.36 * (height * 100 * 0.4)) * 0.45);
}
...
export default function getBMI({
  ...
  return {
    bmi,
    ideal_weight:
      Math.abs(getIdealWeight({ fitness_goal, gender, height }) - weight) < 4
        ? weight
